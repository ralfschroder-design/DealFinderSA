...
| 2026-06-11 | **Plan 10 — mileage-aware (condition) deal scoring.** `build_market_reference` now tracks per-cohort `mileage_median`; `score_listing` folds a bounded **±20** adjustment into `deal_score` when the cohort has ≥2 mileaged peers — lower-than-peer km lifts the score, higher-than-peer km discounts an otherwise-cheap listing (so "cheap because high-mileage" isn't mis-flagged as a deal). `deal_delta_*` stay pure price; no-mileage = unchanged. No migration. TDD; **211 tests passing** (+6). Margin estimate deferred (needs cost assumptions). Plan: `docs/plans/2026-06-11-plan-10-mileage-aware-scoring.md`. |
| 2026-06-11 | **Density + visibility.** Gumtree `max_pages` 1→3 (more listings per run → more cohorts reach the ≥2 scoring threshold). Local UI cards now surface **mileage + seller type** (makes Plan 8 data useful for triage). Investigated **automart.co.za**: reachable, no anti-bot, but `/cars` is a JS-rendered SPA (listings via a backend API, not in initial HTML) → future source needing API-discovery/headless, not a static scrape. 212 tests passing. |
...
| `docs/plans/2026-06-11-plan-10-mileage-aware-scoring.md` | Plan 10 — mileage-aware (condition) deal scoring |
